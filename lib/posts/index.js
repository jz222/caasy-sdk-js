...
const getById = async (postId) => {
    ...
    // ensure there is an elements property
    if (!data.elements) {
        data.elements = [];
    }
    
    for (let i = 0; i < data.elements.length; i++) {
        if (data.elements[i].type === 'image') {
            data.elements[i].imageUrls = generateImageUrls(data.elements[i].imageName);
        }
        if (data.elements[i].type === 'card') {
            data.elements[i].imageUrls = generateImageUrls(data.elements.imageName);
        }
    }
